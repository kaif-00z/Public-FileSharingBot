...
from .database import del_stored_iteam, get_stored_iteam, info_own_iteam, store_iteam

PREMIUM = """
**Premium Membership Advantages**
cost - $5
• You Can Store Unlimited Files
• Multiple Authorised User
• Faster Bot
• Only You and Authorized Can use /generatelink and /revoke command

__~contact @kaif_00z__
"""
...
async def gen_link(event):
    ...
    async with event.client.conversation(event.sender_id) as cv:
        ...
        while True:
            ...
            X.append(fwd.id)
            if len(X) > 3:
                await event.reply("You can add only 3 file")
                return await event.reply(PREMIUM)
            await x.reply("`Added`")
    u_id = (
        (b64encode("".join(choices(ascii_letters + digits, k=8)).encode("ascii")))
        .decode("ascii")
        .replace("=", "")
    )
    store_iteam(u_id, X)
    info_own_iteam(u_id, event.sender_id)
